export default class CustomConfig { 
  ...
  setInternalMode(){
    const internalQueryParam = this.searchParam("internal");
    if(internalQueryParam){
      this.settingsRepo.setBoolean("internalMode",internalQueryParam=="true");
    }

    this.internalMode = this.settingsRepo.getBoolean("internalMode",false);
  }
  ...
  isMobileOrTablet(){
    if(this.searchParam("mobile")) return true;
    let check = false;
